...
class PairedDataset(Dataset):
    ...
    def __init__(self, datasets, n_elements, max_iters, same=True,
                 pair_based_transforms=None, seed=1337, prob=0.5):
        super().__init__()
        ...
        self.history_mask_list = [None] * max_iters
        self.max_iters = max_iters
        self.pair_based_transforms = pair_based_transforms
        if same:
            if isinstance(self.n_elements, int):
                datasets_indices = [self.rand_gen.randrange(self.n_datasets)
                                    for _ in range(self.max_iters)]
                self.indices = [[(dataset_idx, data_idx)
                                 for data_idx in self.rand_gen.choices(range(self.n_data[dataset_idx]),
                                                                k=self.n_elements)]
            ...
            if isinstance(self.n_elements, list):
                self.indices = [[(dataset_idx, data_idx)
                                 for i, dataset_idx in enumerate(
                                     self.rand_gen.sample(range(self.n_datasets), k=len(self.n_elements)))
                                 for data_idx in self.rand_gen.sample(range(self.n_data[dataset_idx]),
                                                               k=self.n_elements[i])]
                ...
                self.indices = [[(dataset_idx, self.rand_gen.randrange(self.n_data[dataset_idx]))
                                 for dataset_idx in self.rand_gen.sample(range(self.n_datasets),
                                                                  k=n_elements)]
    ...
    def __getitem__(self, idx):
        ...
        if sample[0]['label'][cls].max() == 0 or sample[1]['label'][cls].max() == 0:
            ...
            if self.history_mask_list[idx] is None:
                ...
                if self.rand_gen.random()>self.prob:
