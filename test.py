...
def save(save_dir, support_rgb, support_mask, query_rgb, pred, iter_i):
    for srgb, smask, qrgb, p in \
            zip(support_rgb, support_mask, query_rgb, pred):
        cv2.imwrite(save_dir+'/sprt/'+'%05d'%iter_i+'.png', srgb.numpy())
        cv2.imwrite(save_dir+'/qry/'+'%05d'%iter_i+'.png', qrgb.numpy())
        cv2.imwrite(save_dir+'/sprt_lbl/'+'%05d'%iter_i+'.png', smask[0].cpu().numpy())
        cv2.imwrite(save_dir+'/qry_pred/'+'%05d'%iter_i+'.png', p.cpu().numpy())
...
parser.add_argument('-model_type',
                    type=str,
                    help='Model type vanilla, coatt, coatt with neural word embeddings',
                    default='vanilla')
...
data_dir = '/home/msiam/Datasets/VOCdevkit/VOC2012'
...
cudnn.enabled = True
# Create network.
model = Res_Deeplab(num_classes=num_class, model_type=options.model_type)
...
valloader = data.DataLoader(inferset, batch_size=options.bs, shuffle=False, num_workers=0,#4,
                            drop_last=False)
