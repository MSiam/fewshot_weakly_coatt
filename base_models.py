...
class FilMedBottleneck(Bottleneck):
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None):
        super(FilMedBottleneck, self).__init__(inplanes, planes, stride, dilation,
                                               downsample)
        self.film = FiLM()
    ...
    def forward(self, x):
        ...
        out = self.conv3(out)
        out = self.bn3(out)
        out = self.film(out, x['gammas'], x['betas'])

        if self.downsample is not None:
            residual = self.downsample(x['x'])

        out += residual
        out = self.relu(out)

        out = {'x': out, 'gammas': x['gammas'], 'betas': x['betas']}
...
class FilMGen(nn.Module):
    def __init__(self):
        ...
        self.gen_gammas_256 = nn.Linear(300, 256)
        self.gen_betas_256 = nn.Linear(300, 256)

        self.gen_gammas_1024 = nn.Linear(300, 1024)
        self.gen_betas_1024 = nn.Linear(300, 1024)
