...
def main(_run, _config, _log):
    ...
    for i_iter, sample_batched in enumerate(trainloader):
        ...
        sample_class = sample_batched['class_ids']
        # Forward and Backward
        optimizer.zero_grad()
        query_pred = model(support_images, support_fg_mask, support_bg_mask,
                                       query_images, sample_class)
        
        query_pred = F.softmax(query_pred,dim=1)
        query_pred = nn.functional.interpolate(query_pred,size=input_size, mode='bilinear',align_corners=True)#upsample
